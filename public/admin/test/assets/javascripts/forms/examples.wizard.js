	...
	*/
	...
	$('#w1').bootstrapWizard({
		...
		onNext: function( tab, navigation, index, newindex ) {
			...
			if(Number($('#w1-noq').val()) > Number($('#w1-noqa').val())){
				new PNotify({
					title: 'Error',
					text: 'Questions to be answered can not be greater than questions to be added',
					type: 'custom',
			...
			if(Number($('#w1-mpq').val()) * Number($('#w1-noq').val()) > Number($('#w1-tm').val())){
				new PNotify({
					title: 'Error',
					text: 'The mark per question multiplied by the number of question to be answered is greater than the total mark.',
					type: 'custom',
					addclass: 'notification-danger',
					icon: 'fa fa-check'
				});
