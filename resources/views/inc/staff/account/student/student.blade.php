			                                        ...
			                                        <div class="modal-content">
			                                            ...
			                                            <div class="modal-body">
				                                        	...
				                                        	<div class="accrodion-regular">
								                                <div id="accordion{{$pay->id}}s{{$student->id}}">
								                                    <div class="card bg-success">
								                                        <div class="card-header" id="headingTen">
								                                            <h5 class="mb-0">
								                                               <button class="btn btn-link text-white collapsed" data-toggle="collapse" data-target="#collapse{{$pay->id}}s{{$student->id}}" aria-expanded="false" aria-controls="collapse{{$pay->id}}s{{$student->id}}" style="background-color: green; color: white;">
								                                                 <span class="fas fa-angle-down mr-3"></span>Edit Student Payment
								                                               </button>
								                                              </h5>
								                                        </div>
								                                        <div id="collapse{{$pay->id}}s{{$student->id}}" class="collapse" aria-labelledby="headingTen" data-parent="#accordion{{$pay->id}}s{{$student->id}}" style="">
								                                            <div class="card-body">
								                                            	<form id="pays{{$pay->id}}s{{$student->id}}">
								                                            		{{ csrf_field() }}
                                        											{{ method_field('PUT') }}
                                        											<input type="hidden" class="form-control" name="id" value="{{$account->id}}"  id="id">
								                                            		<div class="row">
										                                            	<div class="col-md-4">
										                                        			<strong style="font-size: 15px; color: white;">Amount Payed: </strong>
										                                        		</div>
										                                        		<div class="col-md-8">
												                                        	<input type="number" id="payment{{$pay->id}}s{{$student->id}}" style="background-color: transparent;" class="form-control" name="payment{{$pay->id}}s{{$student->id}}">
										                                        		</div>
										                                        	</div>
										                                        	<br>
										                                        	<div class="row">
										                                        		<div class="col-md-8">
										                                        			<input type="hidden" id="student{{$pay->id}}s{{$student->id}}" class="form-control" name="student{{$pay->id}}s{{$student->id}}" value="{{$student->id}}">
										                                        			<input type="hidden" id="pay{{$pay->id}}s{{$student->id}}" class="form-control" name="pay{{$pay->id}}s{{$student->id}}" value="{{$pay->id}}">
										                                        			<input type="hidden" id="class{{$pay->id}}s{{$student->id}}" class="form-control" name="class{{$pay->id}}s{{$student->id}}" value="{{$class->id}}">
										                                        		</div>
										                                        	</div>
