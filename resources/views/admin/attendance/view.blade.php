    ...
    @extends('layouts.admin')
        ...
        @section('content')
            <div class="container-fluid dashboard-content">
                ...
                <div class="row">
                    <div class="col-xl-8">
                        <a href="{{route('attendances.index')}}" class="btn btn-danger"> < Back </a>
                    </div>
                            ...
                            <div class="tab-content" id="myTabContent5">
                                            ...
                                            <div class="row">
                                                <p class="lead col-md-7">
                                                    @if(count($classes) > 0)
                                                        @foreach($classes as $class)
                                                            @if($class->id == $attendance->classid)
                                                                Attendance For <a href="{{route('classes.show', $class->id)}}">{{$class->name}}</a>
                                                            @endif
                                                        @endforeach
                                                    @else
                                                        <p style="color: red;">None</p>
                                                                ...
                                                                @if($account->studentid == $student->id && $account->classid == $attendance->classid)
                                                                    @if(count($attendance_lists) > 0)
                                                                        @foreach($attendance_lists as $attendance_list)
                                                                            @if($attendance_list->attendanceid == $attendance->id && $attendance_list->studentid == $student->id)
                                                                                <tr style="border: 3px solid; border-color: lightgray;">
                                        ...
                                        <div class="col-md-12">
                                            <div class="row">
                                                ...
                                                <div class="col-md-2"><br>
                                                    <a href="/main_controller_panel/attend/download/{{$attendance->id}}" style="padding: 10px; padding-left: 20px; padding-right: 20px; margin-bottom: 20px;" class="btn btn-success" target="_blank">Print</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="tab-pane fade" id="profile-simple" role="tabpanel" aria-labelledby="profile-tab-simple">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <p><a href="{{route('attendances.edit', $attendance->id)}}"><i class="fas fa-edit"></i> Edit Attendance</a></p>
                                            </div>
                                            <div class="col-md-6">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <form id="delete-form-{{ $attendance->id }}" method="post" action="{{ route('attendances.destroy',$attendance->id) }}" style="display: none">
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}
                            </form>
