	...
	@extends('layouts.admin')
		...
		@section('content')
			<div class="container-fluid dashboard-content">
	            ...
	            <div class="row">
	                <div class="col-xl-8">
	                    <a href="{{url('main_controller_panel/attendances/create')}}" class="btn btn-success">Add New Attendance</a>
		                                ...
		                                <tbody style="position: relative;">
		                                	@foreach($attendances as $attendance)
			                                    <tr>
			                                    	<td></td>
			                                    	<td>
			                                        	<a href="{{route('attendances.show', $attendance->id)}}">{{$attendance->m}} {{$attendance->d}}, {{$attendance->y}}</a>
			                                        </td>
			                                        <td>
			                                        	@if(count($classes) > 0)
			                                        		@foreach($classes as $class)
			                                        			@if($class->id == $attendance->classid)
			                                        				<a href="{{route('attendances.show', $attendance->id)}}">{{$class->name}}</a>
			                                        			@endif
			                                        		@endforeach
			                                        	@else
			                                        ...
			                                        <td>
			                                            ...
			                                            <div class="collapse submenu" id="submenu-{{$attendance->id}}" style="position: absolute; background-color: white; border: 1px solid; border-color: #b8bce0; z-index: 1000;">
					                                        <!-- item-->
				                                            <a href="{{route('attendances.show', $attendance->id)}}" class="dropdown-item" style="color: #71748d;"><i class="fas fa-eye"></i> View</a>
				                                            <!-- item-->
				                                            <a href="{{route('attendances.edit', $attendance->id)}}" class="dropdown-item" style="color: #71748d;"><i class="fas fa-edit"></i>Edit</a>
				                                            <!-- item-->
