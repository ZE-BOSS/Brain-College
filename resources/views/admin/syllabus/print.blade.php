                                        ...
                                        @foreach($classes as $class)
                                            @foreach($subjects as $subject)
                                                @if($subject->id == $syllabus->subjectid && $class->id == $syllabus->classid)
                                                    <p style="font-size: 35px; padding: 20px; text-align: center;">Syllabus For <a href="{{route('subjects.show', $subject->id)}}"> {{$subject->name}}</a>, <a href="{{route('classes.show', $class->id)}}"> {{$class->name}}</a></p>
                                                @endif
                                            @endforeach
                                        @endforeach
                                    @else
