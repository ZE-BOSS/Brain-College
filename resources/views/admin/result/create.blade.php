...
<html >
    <head>
        @include('inc.admin.result.create.index')
    </head>
    <body id="reload">
        <!-- ============================================================== -->
        <!-- main wrapper -->
                                            ...
                                            @if(count($students) > 0)
                                                @foreach($students as $student)
                                                    @if($student->id == $result->studentid)
                                                        <a href="{{route('students.show', $student->id)}}" class="btn-danger" style="padding: 10px;"> < Back</a>
                                                    @endif
                                                @endforeach
                                            @else
                                            ...
                                            @if(count($r_tables) > 0)
                                                @foreach($r_tables as $r_table)
                                                    @if($result->tablerand == $r_table->rand)
                                                        <form action="{{ route('results.update', $result->id) }}" enctype="multipart/form-data" method="post">
                                                            @csrf
                                                            {{ method_field('PUT') }}
                                                            <div class="col-md-12">
                                                                <div class="row">
                                                ...
                                                @endforeach
                                            @else
                                                <form action="{{ route('results.store') }}" enctype="multipart/form-data" method="post">
                                                    @csrf
                                                    <div class="col-md-12">
                                                        <div class="row">
