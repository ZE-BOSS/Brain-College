	...
	@extends('layouts.staff')
		...
		@section('content')
			<div class="container-fluid dashboard-content">
	            ...
	            <div class="row">
	                <div class="col-xl-8">
	                    <a href="{{url('staff_portal/staff_staff/create')}}" class="btn btn-success">Employ New Staff</a>
	                </div>
		                                ...
		                                <tbody style="position: relative;">
		                                	@foreach($staffs as $staff)
			                                    <tr>
			                                        ...
			                                        <td><a href="{{route('staff_staff.show', $staff->id)}}">{{$staff->username}}</a></td>
			                                        <td><a href="{{route('staff_staff.show', $staff->id)}}">{{$staff->firstname}} {{$staff->othernames}}</a></td>
			                                        <td>{{$staff->email}}</td>
			                                        ...
			                                        <td>
			                                            ...
			                                            <div class="collapse submenu" id="submenu-{{$staff->id}}" style="position: absolute; background-color: white; border: 1px solid; border-color: #b8bce0; z-index: 1000;">
					                                        <!-- item-->
				                                            <a href="{{route('staff_staff.show', $staff->id)}}" class="dropdown-item" style="color: #71748d;"><i class="fas fa-eye"></i> View</a>
				                                            <!-- item-->
				                                            <a href="{{route('staff_staff.edit', $staff->id)}}" class="dropdown-item" style="color: #71748d;"><i class="fas fa-edit"></i>Edit</a>
				                                            <!-- item-->
			                                    ...
			                                    <div class="modal" id="exampleModal{{$staff->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="display: none;">
				                                    <div class="modal-dialog" role="document">
				                                        <div class="modal-content">
				                                            ...
				                                            <div class="modal-footer">
				                                                <form id="delete-form-{{ $staff->id }}" method="post" action="{{ route('staff_staff.destroy',$staff->id) }}" style="display: none">
				                                                  {{ csrf_field() }}
				                                                  {{ method_field('DELETE') }}
				                                                </form>
				                                ...
				                                <div class="modal fade show" id="showModal{{$staff->id}}" tabindex="-1" role="dialog" aria-labelledby="showModalLabel" style="display: none;">
				                                    <div class="modal-dialog" role="document">
				                                        <div class="modal-content">
				                                            <div class="modal-header">
				                                                <h5 class="modal-title" id="exampleModalLabel"><a href="{{route('staff_staff.show', $staff->id)}}">{{$staff->firstname}} {{$staff->othernames}}</a></h5>
				                                                <a href="#" class="close" data-dismiss="modal" aria-label="Close">
				                                                    <span aria-hidden="true">×</span>
				                                                </a>
				                                            </div>
